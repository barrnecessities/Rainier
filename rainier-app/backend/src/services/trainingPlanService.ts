...
export function getWeekPlan(summitDateIso: string, today = new Date()): WeekPlan {
...
export function readinessFromWhoop(recovery: number, strain: number, sleepPerformance: number) {
  const score = Math.max(
    1,
    Math.min(100, Math.round(recovery * 0.45 + (100 - Math.min(100, strain * 3.5)) * 0.25 + sleepPerformance * 0.3))
  );
